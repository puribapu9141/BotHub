...
async def _(event):
    ...
    if not USER_AFKB:  # pylint:disable=E0602
        ...
        if reason:
            await borg.send_message(event.chat_id, f"**My Master 👑 {DEFAULTUSER} 👑 is Going AFK!** __Because My Master is {reason}__")
        else:
            await borg.send_message(event.chat_id, f"**My Master 👑 {DEFAULTUSER} 👑 is Going AFK!** __Because My Master is {AFKSK}__")
    if user.last_name:
...
async def on_afkb(event):
    ...
    if USER_AFKB and not (await event.get_sender()).bot:  # pylint:disable=E0602
        ...
        message_to_reply = f"My Master {DEFAULTUSER} is {reason} **Since** {total_afkb_time}" + \
            f"\n__and HE May be Back Soon__\n**Just Wait for My Master's Reply__\n**Thanks for The Patients" \
            if reason \
            else f"My Master 👑 {DEFAULTUSER} 👑 is **afk Since** {total_afkb_time}. \nand My Master has Left Some Word for You 🙃: \n{AFKSK}\n`.` "
