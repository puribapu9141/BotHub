...
async def fgban(event):
    ...
    fgbanVar = event.text
    fgbanVar = fgbanVar[6:]
    mentions = "`Warning!! User is Globally 𝘽𝘼𝙉𝙉𝙀𝘿 By Pratik Goswami PM...\n`"
    no_reason = "__Reason: forgot to add reason. __"
    await event.edit("**Kicking this person out ❗️🖕☠️**")
    asyncio.sleep(3.5)
    ...
    if event.reply_to_msg_id:
        ...
        if idd == 236390091:
            ...
            jnl=("`Warning!! `"
                  "[{}](tg://user?id={})"
                  "` Globally 𝘽𝘼𝙉𝙉𝙀𝘿 By Pratik Goswami PM...\n\n`"
                  "**User's Name: ** __{}__\n"
                  "**ID : ** `{}`\n"
        ...
        mention = "`Warning!! User is Globally 𝘽𝘼𝙉𝙉𝙀𝘿 By Pratik Goswami PM...\nReason: forgot to add reason. `"
...
async def unfgban(event):
    ...
    mentions = "`Warning!! User is Globally UNBANNED By Pratik Goswami PM...\n`"
    no_reason = "__Reason: forgot to add reason. __"
    await event.edit("**allowing this person in ❗🙄☠️**")
    asyncio.sleep(3.5)
    chat = await event.get_input_chat()
    async for x in borg.iter_participants(chat, filter=ChannelParticipantsAdmins):
        mentions += f""
    ...
    if event.reply_to_msg_id:
        ...
        if idd == 236390091:
            ...
            jnl=("`Warning!! `"
                  "[{}](tg://user?id={})"
                  "` Globally UNBANNED By Pratik Goswami PM...\n\n`"
                  "**User's Name: ** __{}__\n"
                  "**ID : ** `{}`\n"
                ).format(firstname, idd, firstname, idd)
            if usname == None:
        ...
        mention = "`Warning!! User is Globally UNBANNED By Pratik Goswami PM...\nReason: forgot to add reason. `"
        await event.reply(mention)
    await event.delete()
